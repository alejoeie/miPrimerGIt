...
for i in range(filas):
    for k in range(columnas):
        ...
        if i == k:
            z_kk[i, k] = Rkk + Rkk_prima + z*(4*mu*np.pi*f*np.log((Dkk_prima/dkk[i,k])))
        else:
            z_kk[i, k] = Rkk_prima + z*(4*mu*np.pi*f*np.log(MatDkk_prima[i,k]/dkk[i,k])) 
print('La matriz de impedancias es: \n', z_kk)

#Nombramos Za
#Matriz de 6x6
Za = np.zeros([3, 3], dtype=np.complex_)
...
#Zb de tamaño 6x2
Zb = np.zeros([3,2], dtype = np.complex_)
for i in range(3):
    for j in range(2):
        Zb[i, j] = z_kk[i, j]
print('Zb:', Zb)
...
#Zc matriz de 2x6
Zc = np.zeros([2,3], dtype = np.complex_)
for i in range(2):
    for j in range(3):
        Zc[i, j] = z_kk[i, j]
print('Zc:', Zc)
...
A_kk = np.matrix([[a11, a12, a13, a14, a15, a16, a17, a18], 
                  ...
                  [a81, a82, a83, a84, a85, a86, a87, a88]],dtype = float, copy=True)
